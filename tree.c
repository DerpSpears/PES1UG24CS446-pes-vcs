...
static int build_tree_recursive(IndexEntry *entries, int count, int depth, ObjectID *id_out) {
    ...
    while (i < count) {
        ...
        const char *path_remainder = entry->path + depth;
        const char *slash = strchr(path_remainder, '/');
        
        if (!slash) {
            TreeEntry *te = &curr_tree.entries[curr_tree.count++];
            te->mode = entry->mode;
            te->hash = entry->hash;
            strcpy(te->name, path_remainder);
            i++;
        } else {
            size_t dir_len = slash - path_remainder;
            char dir_name[256];
            strncpy(dir_name, path_remainder, dir_len);
            dir_name[dir_len] = '\0';
            
            int j = i;
            while (j < count) {
                const char *rem = entries[j].path + depth;
                if (strncmp(rem, dir_name, dir_len) == 0 && rem[dir_len] == '/') {
                    j++;
                } else {
                    break;
                }
    ...
    void *tree_data;
    size_t tree_len;
    if (tree_serialize(&curr_tree, &tree_data, &tree_len) != 0) return -1;
